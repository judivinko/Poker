...
function nextOccupiedIndex(order, from){
  const n = order.length; if(n===0) return -1;
  let idx = order.indexOf(from); if(idx<0) idx = 0;
  for(let k=1;k<=n;k++){ const cand = order[(idx+k)%n]; if(cand!==-1) return cand; }
  return -1;
}
function activeOrder(table_id){
  const rows = liveSeats(table_id); return rows.map(r=>r.seat_index);
}
function initHand(table){
  const order = activeOrder(table.id);
  if(order.length < 2) return null;
  const g = GAME[table.id] || (GAME[table.id]={});
  g.deck = newDeck(); g.board = []; g.hole = {}; g.pot = 0; g.allin = new Set(); g.street = "preflop";
  ...
  g.bet = bbAmt; g.minRaise = table.bb;
  for(const i of order){ const c1=g.deck.pop(), c2=g.deck.pop(); g.hole[i]=[c1,c2]; }
...
function advanceStreet(table, g){
  ...
  else { g.street="showdown"; }
  g.bet=0; g.minRaise=table.bb; g.committed={};
  g.toAct = activeOrder(table.id);
  if(g.street!=="preflop"){
    const afterDealer = nextOccupiedIndex(g.toAct, g.dealer);
    g.toAct = g.toAct.slice(g.toAct.indexOf(afterDealer)).concat(g.toAct.slice(0,g.toAct.indexOf(afterDealer)));
  }
...
function everyoneFoldedExceptOne(g){
  const alive = g.toAct.filter(i=>!g.allin.has(i)); return alive.length<=1;
}

// ===== ČETVRTI DIO (Evaluator – JEDNA verzija) =====
...
app.get("/api/logout",(req,res)=>{
  res.clearCookie("uid", { httpOnly:true, sameSite:"lax", secure:isReqSecure(req), path:"/" });
  res.json({ ok:true });
});
...
app.get("/api/table/state",(req,res)=>{
  ...
  res.json({
    ...
    dealer: gs.dealer|0, acting: gs.acting|0,
    sb_i: g ? g.sb_i : -1, bb_i: g ? g.bb_i : -1,
    seats: seats.map(x=>({ seat_index:x.seat_index, user_id:x.user_id||null, stack:x.stack|0, email:x.email||null, avatar:x.avatar||null })),
    me_seat, my_hole, call_amt, min_bet, min_raise, can_check, min_buy, max_buy
  });
});
app.post("/api/table/action",(req,res)=>{
  ...
  const nextAct = ()=> {
    ...
    for(let k=1;k<=g.toAct.length;k++){
      ...
      if(g.allin.has(cand)) continue; next = cand; break;
    }
    if(next===-1) next = seat;
  ...
  if(action==="fold"){
    ...
    if(everyoneFoldedExceptOne(g)){
      pushPotFromCommitted(g); g.street="showdown"; showdownAndPayout(t,g);
      db.prepare("UPDATE game_state SET street=?, board=?, pot=?, acting=? WHERE table_id=?").run("waiting","",0,-1,table_id);
      delete GAME[table_id];
      res.json({ ok:true }); broadcastTable(table_id); return;
    }
    nextAct(); res.json({ ok:true }); broadcastTable(table_id); return;
  }
  ...
  if(action==="call"){
    ...
    if(roundAllCalledOrAllIn(g)){
      pushPotFromCommitted(g);
      const gs2=db.prepare("SELECT * FROM game_state WHERE table_id=?").get(table_id);
      g.board = parseBoard(gs2.board); g.street = gs2.street;
      advanceStreet(t,g);
      if(g.street==="showdown"){
        showdownAndPayout(t,g);
        db.prepare("UPDATE game_state SET street=?, board=?, pot=?, acting=? WHERE table_id=?").run("waiting","",0,-1,table_id);
  ...
  if(action==="bet" || action==="raise"){
    ...
    if(action==="bet"){
      ...
      targetBet = reqAmt; g.minRaise = Math.max(baseMinRaise, t.bb);
    }else{
      const raiseSize = Math.max(reqAmt, baseMinRaise);
      targetBet = currentBet + raiseSize; g.minRaise = raiseSize;
    }
    const toPutRaw = Math.max(0, targetBet - myComm);
    let toPut = Math.min(toPutRaw, g.stacks[seat]||0);
...
<title>Poker Lobby</title><link rel="stylesheet" href="/app.css">
</head><body style="background:#0b1220;color:#e5e7eb;font-family:system-ui,Segoe UI,Roboto,Arial,sans-serif">
...
function roomBroadcast(table_id,msg){
  ...
  wss.clients.forEach(c=>{ if(c.table_id===table_id && c.readyState===1) c.send(data); });
}
function broadcastLobby(){
  const data=JSON.stringify({type:"lobby"});
  wss.clients.forEach(c=>c.readyState===1 && c.send(data));
}
function broadcastTable(table_id){ roomBroadcast(table_id,{type:"update"}); }
wss.on("connection",(ws)=>{
  ws.table_id=null;
  ws.on("message",(m)=>{
    try{ const d=JSON.parse(m); if(d.type==="join-table") ws.table_id=d.table_id|0; }catch{}
  });
});
